...
const WhyKnowLumi = () => {
  return (
    <div className="flex flex-col items-center gap-14 mt-24">
      <div className="flex flex-col items-center gap-3">
        <span className=" text-[#88DB1B] font-bold text-xl tracking-wide">
          What makes us stand out!
        </span>
        <h2 className="font-medium text-5xl tracking-wide">
          Why join KnowLumi?
        </h2>
      </div>
      <div className="flex gap-14 w-full  items-center justify-center">
        {WhyKnowlumi.map((item, i) => (
          <div
            key={`why_${i}`}
            className="flex flex-col items-center py-6 px-7 bg-white justify-between rounded-xl w-[27.5rem] h-[34.5rem]"
          >
            ...
            {item.points.map((point, index) => (
              <>
                <span
                  key={`point-${index + 1}`}
                  className="text-black font-light text-sm w-96"
                >
                  {point}
                </span>
                <span className="h-[1px] w-96 bg-[#0101011A]"></span>
              </>
            ))}
          </div>
        ))}
